...
for i = 1:length(N_SATS_VEC)
    ...
    if mean(rule_rewards) > best_rule_reward
        ...
        SWEEP_RESULTS.best_Rule_rewards = reward_vec_mcts;
    ...
    for s = 1:length(n_sats_vec)
        plot(comms_reward_vec, reshape(mean(SWEEP_RESULTS.FS_results(1,:,s,:), 4), [length(comms_reward_vec), 1]), '.-', 'markersize', 25, 'linewidth', 1.25,  'displayname', ['# Satellites: ' num2str(n_sats_vec(s))]); hold on
    end
    legend
